  ...
  constructor(
    ...
    try {
      const hash = ethers.TypedDataEncoder.hash(
        emptyEIP712.domain,
        { UserDecryptRequestVerification: (emptyEIP712 as any).types.UserDecryptRequestVerification },
        emptyEIP712.message as any
      );
...
export class FhevmDecryptionSignature {
  ...
  static checkIs(s: unknown): s is FhevmDecryptionSignatureType {
    ...
    for (let i = 0; i < (s as any).contractAddresses.length; ++i) {
      ...
      if (!(s as any).contractAddresses[i].startsWith("0x")) return false;
    }
    if (
      !(
        "userAddress" in s &&
  ...
  static async loadOrSign(
    ...
    const userAddress = (await (signer as any).getAddress()) as `0x${string}`;
